...
class DashboardController extends Controller
{
    public function index($mt4_account = null) {
        ...
        $secondSmallestFormonth = Eombalance::where('account_id', $account->id)
            ->where('formonth', '<', $lastmonth)
        ...
        Foreach ($eombalances as $eombalance) {
            ...
            if($eombalance->eombalance > $prevbalance){
                $pl = $eombalance->eombalance - $prevbalance;
            }else $pl = $prevbalance - $eombalance->eombalance;
            // $monthlypl[$eombalance->formonth] = $pl;
            $monthlypl[$eombalance->formonth] = $eombalance->eombalance;
            $dbvar['current_balance'] = number_format($eombalance->eombalance);
        }
        $percentage = 0;
        if(isset($monthlypl[$lastmonth])){
           
            $percentage = ($monthlypl[$lastmonth] / $eombalance4view[$secondSmallestFormonth]) * 100;
        }
